...
DROP VIEW IF EXISTS WypozyczenieView;
CREATE VIEW WypozyczenieView AS (
    SELECT W.id AS "Id Wypożyczenia", W.id_klient AS "Id Klienta", Kl.pesel AS "Pesel", W.id_ksiazka AS "Id Książki",
    Ks.tytul AS "Tytuł", W.data_wypozyczenia AS "Data Wypożyczenia", W.data_oddania AS "Data Oddania", W.aktualne AS "Aktualne"
    FROM "Wypozyczenie" AS W, "Klient" AS Kl, "Ksiazka" AS Ks
    WHERE Kl.id = W.id_klient AND Ks.id = W.id_ksiazka
    ORDER BY W.aktualne DESC
);